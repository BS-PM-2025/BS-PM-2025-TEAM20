...
def login_lec(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                user =  UserRegisterLec.objects.get(username=username)
...
def schedule_meeting_for_lecc(request):
    if request.method == 'POST':
        ...
        return redirect('schedule_meeting_for_lecc')
...
from django.shortcuts import render
from .models import StudentRequest,Appointment, UserRegisterLec, UserRegisterStu1,OfficeHour
...
from django.utils import timezone
def lecc_page(request):
    student_requests = StudentRequest.objects.order_by('-created_at')[:3]
    ...
    # جلب المواعيد الخاصة بالمحاضر
    # موقتًا نحط اسم المحاضر بشكل ثابت، غيّره حسب الحاجة:
    office_hours = OfficeHour.objects.filter(lecturer__username='yaqeen')
    appointments = Appointment.objects.filter(office_hour__in=office_hours).order_by('request_time')[:3]

    return render(request, 'lecc_page.html', {
        'student_requests': student_requests,
        'meetings': meetings,
